...
The `udpsend.py` Python program reads a file (by default `udpsend.txt`) and builds a network packet
...
`px2firmware.txt` then run the `udpsend.py` program as follows:
...
The `udpsend.txt` packet specification file (or the file specified after the `--file`/`-f` command
...
used to document the content of the file. It can also be used as a handy way to
'comment out' command and data lines during the development of the packet
specification file. Comments are ignored by the `udpsend.py` program.
